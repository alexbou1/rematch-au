drop table if exists suburb_lookup;
create table suburb_lookup as
...
from suburb_lookup l
join locality_alias la on la.locality_pid = l.primary_loc_pid;
create index idx_suburb_lookup_primary_loc_pid on suburb_lookup (primary_loc_pid);
-- Created 'suburb_lookup' table;

drop table if exists street_lookup;
...
create index idx_street_lookup_locality_pid on street_lookup (locality_pid);
-- Created 'street_lookup' table;

create temp table suburb_loc_pid_map as
select row_number() over (order by primary_loc_pid)::smallint as mapped_primary_loc_pid, primary_loc_pid
from suburb_lookup
group by primary_loc_pid
order by primary_loc_pid;

create temp table street_loc_pid_map as
select row_number() over (order by street_locality_pid)::int as mapped_street_locality_pid, street_locality_pid
from street_lookup
group by street_locality_pid
order by street_locality_pid;

drop table if exists gnaf.suburb_lookup_vic;
create table gnaf.suburb_lookup_vic as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'VIC'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_nsw as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
...
create table gnaf.suburb_lookup_act as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'ACT'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_qld as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'QLD'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_sa as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'SA'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_tas as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'TAS'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_wa as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
...
create table gnaf.suburb_lookup_nt as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'NT'
order by postcode, primary_loc_pid;
...
create table gnaf.suburb_lookup_ot as
select distinct slpm1.mapped_primary_loc_pid as primary_loc_pid, slpm2.mapped_primary_loc_pid as associated_loc_pid, postcode
from suburb_lookup sl
join suburb_loc_pid_map slpm1 using (primary_loc_pid)
join suburb_loc_pid_map slpm2 on sl.associated_loc_pid = slpm2.primary_loc_pid
where state_abbreviation = 'OT'
order by postcode, primary_loc_pid;
...
create table gnaf.street_lookup_vic as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
where state_abbreviation = 'VIC'
order by locality_pid, street_name, street_locality_pid;
...
create table gnaf.street_lookup_nsw as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_act as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_qld as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_sa as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_tas as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_wa as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_nt as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.street_lookup_ot as
select distinct slpm2.mapped_street_locality_pid as street_locality_pid, slpm1.mapped_primary_loc_pid as locality_pid, street_name, street_full_name
from street_lookup sl
join suburb_loc_pid_map slpm1 on sl.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
...
create table gnaf.australian_full_addresses_vic as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
    number_first_suffix, number_last_prefix, number_last, number_last_suffix, street_name,
    street_class_code, street_class_type, street_type_code, street_suffix_code, street_suffix_type,
    locality_name, state_abbreviation, postcode, latitude, longitude, confidence, alias_principal,
    primary_secondary, address
from australian_full_addresses a
join suburb_loc_pid_map slpm1 on a.locality_pid = slpm1.primary_loc_pid
join street_loc_pid_map slpm2 using (street_locality_pid)
where state_abbreviation = 'VIC'
order by street_locality_pid, locality_pid, number_first;
...
create table gnaf.australian_full_addresses_nsw as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_act as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_qld as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_sa as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_tas as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_wa as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_nt as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
...
create table gnaf.australian_full_addresses_ot as
select address_detail_pid, slpm2.mapped_street_locality_pid as street_locality_pid,
    slpm1.mapped_primary_loc_pid as locality_pid, building_name, lot_number_prefix, lot_number,
    lot_number_suffix, flat_type, flat_number_prefix, flat_number, flat_number_suffix, level_type,
    level_number_prefix, level_number, level_number_suffix, number_first_prefix, number_first,
